    ...
    public class Functions
    {
        ...
        public static DenseMatrix Threshold(DenseMatrix inMatrix, int upThreshold, int downThreshold)
        {
            ...
            for (int i = 0; i < (inMatrix.ColumnCount); i++ )
            {
                ...
                {
                    if ((downThreshold<=inMatrix[j,i])&&(inMatrix[j,i]<=upThreshold))
                    {
                        inMatrix[j, i] = 1;
                    }
                    else { inMatrix[j, i] = 0; }
        ...
        public static ArrayList findIndices(DenseMatrix inMatrix, int lowerBound) //checked
        {
        ...
        public static List<DenseMatrix> getNSEWfeatures(DenseMatrix inFrame)
        {
            ...
            ArrayList indices = findIndices(inFrame, 0);
        ...
        /// <param name="inFrame"></param>
        /// <returns></returns>
        public static int[][] getTfeatures(ArrayList inFrames)
        {
            //Assume frame is already logical/binary
        ...
        public static float[,] getCovFeaturesFromSequencePath(ushort[,] recording)
        {
            //Convert into float array
            float[,] fRecording = new float[recording.GetLength(0), recording.GetLength(1)];
            ...
            ArrayList cleanSequence = ExtractHand(fRecording);
            ...
            for(int i =0; i < ((DenseMatrix)cleanSequence[0]).RowCount*cleanSequence.Count; i++)
            {
                ...
                {
                    X[i * ((DenseMatrix)cleanSequence[0]).ColumnCount + j] = j+1;
                }
            }
            ...
            for (int u = 0; u < cleanSequence.Count; u++)
            {
                for (int i = 0; i < ((DenseMatrix)cleanSequence[0]).RowCount; i++)
                {
                    for (int j = 0; j < ((DenseMatrix)cleanSequence[0]).ColumnCount; j++)
                    {
                        Y[(u*((DenseMatrix)cleanSequence[0]).RowCount * ((DenseMatrix)cleanSequence[0]).ColumnCount+i * ((DenseMatrix)cleanSequence[0]).ColumnCount + j)] = i + 1;
                    }
                }
            }
            ...
            for (int u = 0; u < cleanSequence.Count; u++)
            {
                for (int i = 0; i < ((DenseMatrix)cleanSequence[0]).RowCount; i++)
                {
                    for (int j = 0; j < ((DenseMatrix)cleanSequence[0]).ColumnCount; j++)
                    {
                        T[(u*((DenseMatrix)cleanSequence[0]).RowCount * ((DenseMatrix)cleanSequence[0]).ColumnCount+i * ((DenseMatrix)cleanSequence[0]).ColumnCount + j)] = u+1;
                    }
                }
            }
            ...
            for (int frame_num=0; frame_num < recording.GetLength(0); frame_num++)
            {
                ...
                ArrayList n = findIndices(curFrame,0);
                //sort indices into x and y lists
                ArrayList xs = new ArrayList();
                ArrayList ys = new ArrayList();
