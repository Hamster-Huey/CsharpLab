...
namespace CsharpLab
{
    public class Functions
    {
        ...
        public static DenseMatrix ExtractHand(float[,] recording)
        {
            ...
            for (int frameNum = 1; frameNum < numFrames; frameNum++)
            {
                ...
                DenseMatrix depthFrameblah = depthFrameTest;
            ...
            for (int i = 0; i < inMatrix.RowCount; i++)
            {
                for (int j = 0; j < inMatrix.ColumnCount; j++)
                {
                    if (inMatrix[i, j] > lowerBound)
                        indices.Add(j * inMatrix.RowCount + i);
                }
            }
        ...
        public static DenseMatrix getCovFeaturesFromSequencePath(ushort[,] recording, int dataSize)
        {
            ...
            DenseMatrix cleanSequence = ExtractHand(fRecording);
